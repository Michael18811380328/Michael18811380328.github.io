...
功能模块化的实现：复杂应用转化成小模块的实现；小模块间耦合性低，确保功能在一次函数增加的时候，函数的复杂性不会爆炸的形式增加 (n*(n-1))/2 ；界面组件化（一个复杂界面由多个小组件构成，小组件的复用性很好，不同小组件的耦合性降低）；一个复杂任务可以分解成多个小任务，首先增加数量，降低多维度的任务（使用主成分分析的方法将高维度任务的末尾项维度去掉，在高分子有机化学的主成分分析上，8维度的数据可以保证99%以上的信息正确，说明一半以上的数据存在高度耦合）；实现降维操作需要线性代数的基础（自己有思想但是数理基础不行）。
