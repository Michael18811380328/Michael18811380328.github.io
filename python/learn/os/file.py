...
def convert_md_link(file_content, repo_id, username):
    def repl(matchobj):
        ...
        if fileext == '':
            ...
            return '<img class="wiki-image" src="%s" alt="%s" />' % (
...
def can_preview_file(file_name, file_size, repo):
    ...
    if filetype in (TEXT, MARKDOWN, IMAGE) or fileext in get_conf_text_ext():
        ...
        if repo.encrypted:
            error_msg = _(
                u'The library is encrypted, can not open file online.')
            return False, error_msg
...
def view_lib_file_via_smart_link(request, dirent_uuid, dirent_name):
    ...
    dirent_path = posixpath.join(
        parent_path, dirent_name_from_uuid_map.strip('/'))
    if not is_dir:
        redirect_to = reverse('view_lib_file', args=[repo_id, dirent_path])
    else:
        redirect_to = '%s#common/lib/%s/%s' % (
            settings.SITE_ROOT, repo_id, dirent_path.strip('/'))
...
def view_lib_file(request, repo_id, path):
    ...
    is_starred = is_file_starred(
        username, repo_id, path.encode('utf-8'), org_id)
    ...
    file_shared_link = gen_file_share_link(
        fileshare.token) if fileshare else ''
    ...
    if filetype in (TEXT, MARKDOWN) or fileext in get_conf_text_ext():
        ...
        review = get_file_draft_and_related_review(
            repo.id, path, is_draft, has_draft)

        if filetype == MARKDOWN:
            return_dict['protocol'] = request.is_secure() and 'https' or 'http'
            return_dict['domain'] = get_current_site(request).domain
            return_dict['file_content'] = convert_md_link(
                file_content, repo_id, username)
        ...
        if len(img_list) > 1:
            ...
            if cur_img_index != 0:
                img_prev = posixpath.join(
                    parent_dir, img_list[cur_img_index - 1])
            if cur_img_index != len(img_list) - 1:
                img_next = posixpath.join(
                    parent_dir, img_list[cur_img_index + 1])
        ...
        if ENABLE_OFFICE_WEB_APP:
            ...
            if wopi_dict:
                if is_pro_version() and action_name == 'edit':
                    try:
                        if not is_locked:
                            seafile_api.lock_file(
                                repo_id, path, ONLINE_OFFICE_LOCK_OWNER, 0)
                ...
                return_dict['err'] = _(
                    u'Error when prepare Office Online file preview page.')
        ...
        if ENABLE_ONLYOFFICE and fileext in ONLYOFFICE_FILE_EXTENSION:
            ...
            if onlyoffice_dict:
                ...
                send_file_access_msg(request, repo, path, 'web')
                return render(
                    request, 'view_file_onlyoffice.html', onlyoffice_dict)
            else:
                return_dict['err'] = _(
                    u'Error when prepare OnlyOffice file preview page.')
        ...
        error_msg = prepare_converted_html(
            inner_path, file_id, fileext, return_dict)
        if error_msg:
            return_dict['err'] = error_msg
            return render(request, template, return_dict)
...
def _download_file_from_share_link(request, fileshare):
    ...
    if isinstance(fileshare, FileShare) and fileshare.is_dir_share_link():
        ...
        if not req_path:
            messages.error(
                request, _(u'Unable to download file, invalid file path'))
    ...
    if user_traffic_over_limit(fileshare.username):
        messages.error(
            request,
            _(u'Unable to download file, share link traffic is used up.'))
        return HttpResponseRedirect(next)
...
def view_shared_file(request, fileshare):
    ...
    if request.GET.get('raw', '') == '1':
        ...
        if user_traffic_over_limit(shared_by):
            messages.error(
                request,
                _(u'Unable to view raw file, share link traffic is used up.'))
            next = request.META.get('HTTP_REFERER', settings.SITE_ROOT)
            return HttpResponseRedirect(next)
...
def get_file_content_by_commit_and_path(
        request, repo_id, commit_id, path, file_enc):
    try:
...
def text_diff(request, repo_id):
    ...
    current_commit = seafserv_threaded_rpc.get_commit(
        repo.id, repo.version, commit_id)
    if not current_commit:
        return render_error(request, 'bad commit id')

    prev_commit = seafserv_threaded_rpc.get_commit(
        repo.id, repo.version, current_commit.parent_id)
    if not prev_commit:
...
def _office_convert_get_file_id(
        request, repo_id=None, commit_id=None, path=None):
...
def office_convert_query_status(request, cluster_internal=False):
    ...
    ret = {'success': False}
    try:
        ret = query_office_convert_status(
            file_id, doctype, cluster_internal=cluster_internal)
    except Exception as e:
...
_OFFICE_PAGE_PATTERN = re.compile(
    r'^file\.css|file\.outline|index.html|index_html_.*.png|[a-z0-9]+\.pdf$')


def office_convert_get_page(
        request, repo_id, commit_id, path, filename, cluster_internal=False):
    ...
    if cluster_internal:
        ...
        file_id = _office_convert_get_file_id(
            request, repo_id, commit_id, path)
...
def file_access(request, repo_id):
    ...
    if is_org_context(request):
        ...
        events = get_file_audit_events_by_path(
            None, org_id, repo_id, path, start, limit)
    else:
        events = get_file_audit_events_by_path(
            None, 0, repo_id, path, start, limit)
...
def view_media_file_via_share_link(request):
    ...
    err_msg, file_content = get_file_content_from_cache(
        file_id, repo_id, shared_file_name)
    ...
    p = re.compile('(%s)/lib/(%s)/file(.*?)%s\\?raw=1' %
                   (serviceURL, repo_id, image_file_name))
