...
def downloadTMS(video_urls, savename, savepath='videos',
                max_retries=5, headers=None):
    if not video_urls:
        return False
    checkFolder(savepath)
    if headers is None:
    ...
    while True:
        ...
        flag = downloadBASE(video_url, savename='temp_%d.%s' % (i, savename.split(
            '.')[-1]), savepath=savepath, headers=headers, stream=True, verify=False)
        if not flag:
            i -= 1
            max_retries -= 1
        else:
            videos_dict[i] = savepath + \
                '/temp_%d.%s' % (i, savename.split('.')[-1])
    return is_success
# -------------------------------------------------------------------------------------------
...
def downloadM3U8(video_url, savename, savepath='videos'):
    ...
    try:
        ...
        is_success = True
        if os.name == 'posix':
            os.system('ffmpeg -i {} -c copy {}'.format('"%s"' %
                      video_url, os.path.join(savepath, savename)))
        elif os.name == 'nt':
            os.system('ffmpeg.exe -i {} -c copy {}'.format('"%s"' %
                      video_url, os.path.join(savepath, savename)))
        else:
            is_success = False
    return is_success
