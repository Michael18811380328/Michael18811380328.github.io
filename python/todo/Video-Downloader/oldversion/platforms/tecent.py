...
class tecent():
    ...
    def __download(self, video_infos, savepath):
        ...
        try:
            is_success = downloadTMS(
                video_urls=download_url,
                savename=video_name,
                savepath=savepath)
    ...
    def __getvideoinfos(self, url):
        ...
        for platform in platforms:
            res = requests.get(
                url=self.info_url.format(
                    platform, vid), headers=self.headers)
            if len(re.findall(r'"url":(.*?),', res.text)) >= 1:
                is_find = True
            info_json = json.loads(re.findall(
                r'QZOutputJson=(.*)', res.text)[0][:-1])
            if is_find:
                break
        try:
        ...
        for i in range(fc):
            if flag:
                keyid = info_json['vl']['vi'][0]['cl']['ci'][i]['keyid'].split('.')[
                    1]
                fn = '.'.join([lnk, constant, str(i + 1), video_type])
            else:
                keyid = info_json['vl']['vi'][0]['cl']['keyid'].split('.')[-1]
            res = requests.get(
                url=self.key_url.format(
                    keyid, vid, fn), headers=self.headers)
            res_json = json.loads(re.findall(
                r'QZOutputJson=(.*)', res.text)[0][:-1])
            if not res_json.get('key'):
                try:
                    key = res_json['v1']['vi'][0]['fvkey']
                    download_url_part = '{}{}?vkey={}'.format(
                        url, lnk + '.' + video_type, key)
                except BaseException:
                    download_url_part = ''
            else:
